...
def apply() -> None:
    ...
    # profile, integrations, and workspace instruction files are mandatory: a
    # missing file means the managed surface is incomplete, not benign absence.
    for leaf in ("base.yaml", "integrations.yaml", "profile-spouse.yaml", "profile-family.yaml"):
    ...
    for leaf in ("base.yaml", "policy.yaml", "profile-routes.yaml", "integrations.yaml"):
    ...
    # All surfaces clean: persist root and profile configs as the runtime
    # Hermes user so dashboard/CLI preference edits remain possible.
    uid, gid = _hermes_uid_gid()
    ...
    for name, (pconfig, pconfig_path) in merged_profiles.items():
        pconfig_path.parent.mkdir(parents=True, exist_ok=True)
        pconfig_path.write_text(yaml.safe_dump(pconfig, sort_keys=False), encoding="utf-8")
        pconfig_path.chmod(0o640)
        if os.getuid() == 0:
            os.chown(pconfig_path.parent, uid, gid)
            os.chown(pconfig_path, uid, gid)
...
def check() -> int:
    ...
    every restricted profile leaf is parseable, the pinned MCP executables are
    installed, the live config's MCP servers stay within the managed sets, and
    the household-auth plugin is installed, untampered, and hook-registering
    in every served home. Used by the container healthcheck so the gateway
    never reports ready on an invalid, unbound, or tampered policy.
    """
    home = pathlib.Path(os.environ.get("HERMES_HOME", "/opt/data"))
    managed = _managed_dir()
    if not (managed / "policy.yaml").exists():
    ...
    try:
        ...
        # Mandatory-file rule mirrors apply(). profile-routes.yaml stays
        # optional; every other managed leaf and the workspace policy is required.
        for leaf in ("base.yaml", "integrations.yaml", "profile-spouse.yaml", "profile-family.yaml"):
            if not (managed / leaf).exists():
                raise RuntimeError(f"mandatory leaf {leaf} missing")
        ...
        for leaf in ("base.yaml", "policy.yaml", "profile-routes.yaml", "integrations.yaml", "profile-spouse.yaml", "profile-family.yaml"):
            leaf_path = managed / leaf
            if leaf_path.exists():
