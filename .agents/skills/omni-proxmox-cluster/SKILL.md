---
name: omni-proxmox-cluster
description: Form the three-node Proxmox management cluster from standalone hosts — per-host fact collection, seed-vs-join selection, guest evacuation/restore rehearsal, quorum prerequisites, pvecm formation sequence, and Omni Proxmox provider (v0.3.0) prerequisites (connection config, machine class provider data, ISO download, per-endpoint token ACL verification). Use when the user asks to cluster the Proxmox hosts, evacuate/restore guests before joining, verify provider token permissions, or prepare Proxmox for Omni VM provisioning. Don't use for deploying the management Compose stack itself (see omni-management-stack), for Proxmox HA resource configuration, or for ordinary guest administration after clustering.
---

# Proxmox Cluster Formation

Form the three-node Proxmox management cluster from the current standalone
hosts, evacuate and restore existing guests safely, and establish the
prerequisites for the pinned Omni Proxmox provider.

**Hard rules:**
- Every value is site-specific and collected from the live hosts — never
  guessed. Fill the per-host fact table (section 1) before any change.
- **Joining nodes must be empty of guests** before `pvecm add`. Joining
  overwrites `/etc/pve`, guest IDs must not conflict with the cluster, and
  the node inherits the cluster's storage configuration. The documented
  path for a populated joining node is: vzdump every guest → evacuate →
...
Kubernetes/Longhorn (planned) handles workload recovery. This cluster is
for management and config replication only.

## 1. Per-host fact table (rollout inputs)

Collect from each of the three physical hosts before any change. Do not
guess.
...
- **Seed node** (`pvecm create CLUSTERNAME`): may keep its existing guests.
  ...
  need evacuation. Back up the seed node's guests and configuration as
  well; a backup is a backup regardless of which node joins.
- **Joining nodes** (`pvecm add IP-CLUSTER`): must be empty of guests
  before joining (see hard rules).
...
3. **Restore rehearsal**: restore at least one representative guest backup
   to a new VMID (scratch) on the same node or a staging node. The scratch
   VM must be isolated from production services before it boots: use a
   ...
   intact, then stop and delete the scratch VM.
...
- [ ] Final node names and IPs fixed before `pvecm create`. Corosync
      membership is keyed to static addresses. Renaming a cluster node
      after formation is a version-specific procedure; finalize names now
      to avoid it.
...
Execute in order. Do not skip the verification step after each action.
This sequence requires live operator authorization; no live actions are
executed by this skill.

1. **Seed node**: `pvecm create CLUSTERNAME` (choose a unique cluster
   name). Verify with `pvecm status`. The seed node keeps its guests.
2. **Joining node 1** (already evacuated per section 3): on that node,
   `pvecm add IP-SEED-NODE`. This authenticates against the seed node's
   API with root@pam and verifies the SHA-256 certificate fingerprint.
   Verify `pvecm status` on the seed node shows two nodes, quorate.
...
Verify Omni CLI compatibility with this provider version at pin time in
the isolated validation wave.
...
Required fields (provider schema): `cores`, `memory` (MB), `disk_size`
(GB), `storage_selector` (CEL expression, e.g. `name == "local-lvm"`).
Size values come from the plan's capacity gates (migration plan section 3),
not from provider defaults.
...
- GPU host only: `pci_devices` (resource mapping name), `machine_type:
  q35`, `cpu_type: host`, `balloon: false`. Model-data disk as an
...
The provider does not clone from a template. It resolves the Talos ISO
from the Omni image-factory API and downloads it into each node's ISO
storage (`storage.DownloadURL`). Prerequisites:

- [ ] An ISO storage exists on each of the three nodes with free space for
      the Talos ISO.
- [ ] Each Proxmox node has outbound network access to the image-factory
      URL that Omni returns. If a node is offline, pre-seed the ISO in its
      ISO storage manually; the provider checks for an existing ISO before
      downloading.
...
(https://pve.proxmox.com/pve-docs/api-viewer/). The endpoints to check,
from `internal/pkg/provider/provision.go` and
`internal/pkg/provider/ha/manager.go`:
...
For each endpoint, record the required privilege(s) from the API viewer
for the pinned PVE version, then build the smallest ACL set that covers
them. The token secret is shown once at creation; store it in Bitwarden,
never in manifests or Git.

## 7. Blockers and open questions (operator must resolve)

- [ ] Actual PVE version on each host (must be identical across the
      cluster).
- [ ] Current guest/VMID inventory per node with running/stopped state
      (the evacuation list).
- [ ] Storage layout and free space per node; which storage serves the
      Talos VMs and which serves ISOs.
